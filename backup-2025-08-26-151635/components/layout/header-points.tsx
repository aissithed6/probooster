...
export default function HeaderPoints() {
  ...
  const handleWithdrawPoints = () => {
    ...
    try {
      const currentPoints = parseInt(localStorage.getItem('userPoints') || '1000')
      const newPoints = Math.max(0, currentPoints - 500)
      
      localStorage.setItem('userPoints', newPoints.toString())
      setUserPoints(newPoints)
      setPointsValue(newPoints * 10)
