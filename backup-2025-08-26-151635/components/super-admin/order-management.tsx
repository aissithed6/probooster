         ...
         <DialogContent className={`max-w-2xl h-[90vh] flex flex-col transition-all duration-500 ease-out ${animateModal ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
           ...
           <div className={`space-y-6 overflow-y-auto flex-1 pr-2 ${animateContent ? 'opacity-100 transition-all duration-300' : 'opacity-0'}`}>
             ...
             {/* Formulaire de paiement */}
                            <div className={`space-y-4 ${animateContent ? 'opacity-100 transition-all duration-300' : 'opacity-0'}`}>
             ...
             <Button 
               onClick={() => {
                 if (selectedOrderForPayment) {
                   createPaymentRequest(selectedOrderForPayment.vendor.id, [selectedOrderForPayment.id])
                 }
               }}
