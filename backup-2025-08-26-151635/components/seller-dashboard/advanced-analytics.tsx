...
export default function AdvancedAnalytics({
  ...
  const handleExport = async () => {
    setIsExporting(true)
    showSimpleNotification('info', 'Export en cours', `Export des insights ${exportOptions.type} en format ${exportOptions.format}...`)
    
    setTimeout(() => {
      const content = `Insights ${exportOptions.type} - ${new Date().toLocaleDateString('fr-FR')}\n\n`
      const blob = new Blob([content], { type: 'text/plain' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `insights-${exportOptions.type}-${exportOptions.format}.txt`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      
      onExportInsights(exportOptions.type, exportOptions.format)
      showSimpleNotification('success', 'Export réussi', `Insights exportés avec succès`)
  ...
  const handleRefresh = async () => {
    ...
    showSimpleNotification('info', 'Actualisation en cours', 'Actualisation des analyses avancées...')
    
    setTimeout(() => {
      showSimpleNotification('success', 'Actualisation terminée', 'Toutes les analyses ont été actualisées')
      setShowRefreshModal(false)
      setIsRefreshing(false)
    }, 2500)
  }

  // Fonctions pour la génération de rapports
  const handleGenerateReport = async (reportType: string) => {
    setIsGeneratingReport(true)
    showSimpleNotification('info', 'Génération en cours', `Génération du rapport ${reportType}...`)
    
    setTimeout(() => {
      const content = `Rapport ${reportType} - ${new Date().toLocaleDateString('fr-FR')}\n\n`
      const blob = new Blob([content], { type: 'text/plain' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `rapport-${reportType.toLowerCase()}-${reportOptions.format}.txt`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      
      showSimpleNotification('success', 'Rapport généré', `Rapport ${reportType} généré avec succès`)
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xl font-bold text-blue-900">125,000 F CFA</p>
                <p className="text-sm font-medium text-blue-600">(125,000 pts)</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xl font-bold text-purple-900">45,000 F CFA</p>
                <p className="text-sm font-medium text-purple-600">(45,000 pts)</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-4 rounded-lg border border-green-200 bg-green-50">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-green-700 mb-2">Croissance des ventes prévue</h4>
                  <p className="text-sm text-gray-600 mb-3">Vos ventes devraient augmenter de 15% le mois prochain</p>
            ...
            <div className="p-4 rounded-lg border border-orange-200 bg-orange-50">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-orange-700 mb-2">Risque de churn modéré</h4>
                  <p className="text-sm text-gray-600 mb-3">15% de vos clients inactifs risquent de ne pas revenir</p>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500">Confiance</span>
            ...
            <div className="p-4 rounded-lg border border-red-200 bg-red-50">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-red-700 mb-2">Baisse de l'engagement</h4>
                  <p className="text-sm text-gray-600 mb-3">L'engagement sur vos partages a chuté de 25% cette semaine</p>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-500">Confiance</span>
