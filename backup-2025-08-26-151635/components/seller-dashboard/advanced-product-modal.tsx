...
export default function AdvancedProductModal({ isOpen, onClose, product, mode }: AdvancedProductModalProps) {
  ...
  const handleSaveWithStatus = async (status: 'active' | 'inactive' | 'pending') => {
    if (!productData.name) {
    ...
    try {
      // Préparer les données du produit avec le statut
      const productToSave = {
        ...productData,
        status: status,
      ...
      // Simuler la sauvegarde (remplacer par votre API)
      await new Promise(resolve => setTimeout(resolve, 1500))

      // Déterminer le message de succès selon le statut
      let successMessage = ''
      let statusLabel = ''
      
      switch (status) {
        case 'active':
          successMessage = 'Le produit a été créé et est maintenant visible aux clients'
          statusLabel = 'Actif'
          break
        case 'inactive':
          successMessage = 'Le produit a été créé mais reste invisible aux clients'
          statusLabel = 'Inactif'
          break
        case 'pending':
          successMessage = 'Le produit a été créé et est en attente de validation'
          statusLabel = 'En attente'
          break
      }

      addNotification('success', `Produit ${statusLabel} créé !`, successMessage, {
        label: 'Voir le produit',
        onClick: () => {
          // Ici on pourrait rediriger vers la liste des produits
          addNotification('info', 'Redirection', 'Redirection vers la liste des produits...')
        }
      })
      ...
      addNotification('error', 'Erreur de sauvegarde', 'Impossible de sauvegarder le produit. Veuillez réessayer.')
    } finally {
      setIsLoading(false)
    }
  }
