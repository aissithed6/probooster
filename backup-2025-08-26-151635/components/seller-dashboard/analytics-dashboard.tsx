...
export default function AnalyticsDashboard({
  ...
  const [salesData, setSalesData] = useState<SalesData[]>([
    { period: 'Jan', sales: 1200, revenue: 15000, growth: 12.5 },
    { period: 'Fév', sales: 1350, revenue: 16800, growth: 15.2 },
    { period: 'Mar', sales: 1100, revenue: 13800, growth: -8.1 },
    { period: 'Avr', sales: 1600, revenue: 20000, growth: 45.5 },
    { period: 'Mai', sales: 1400, revenue: 17500, growth: -12.5 },
    { period: 'Jun', sales: 1800, revenue: 22500, growth: 28.6 }
  ])
  ...
  const handleViewProductDetails = (productId: number) => {
    ...
    if (product) {
      ...
      showSimpleNotification('info', 'Détails produit', `Ouverture des détails du produit ${product.name}`)
    }
  }
  ...
  const handleViewCustomerProfile = (customerId: string) => {
    ...
    showSimpleNotification('info', 'Profil client', `Ouverture du profil client ${customerId}`)
  }
  ...
  const handleRefreshData = () => {
    ...
    // Simuler le rafraîchissement avec mise à jour réelle des données
    setTimeout(() => {
      // Générer de nouvelles données aléatoires pour simuler l'actualisation
      setSalesData(prev => prev.map(item => ({
        ...item,
        sales: item.sales + Math.round(Math.random() * 100 - 50),
        revenue: item.revenue + Math.round(Math.random() * 1000 - 500),
        growth: item.growth + (Math.random() > 0.5 ? 1 : -1)
      })))
      
      setProductPerformance(prev => prev.map(product => ({
        ...product,
        sales: product.sales + Math.round(Math.random() * 20 - 10),
        revenue: product.revenue + Math.round(Math.random() * 1000 - 500),
        growth: product.growth + (Math.random() > 0.5 ? 1 : -1)
      })))
  ...
  const handleGenerateReport = (type: string) => {
    ...
    showSimpleNotification('info', 'Génération de rapport', `Préparation du rapport ${type}...`)
  }
  ...
  const handleGenerateReportConfirm = () => {
    ...
    showSimpleNotification('info', 'Génération en cours', `Génération du rapport ${selectedReportType}...`)
    
    // Simuler la génération avec progression
    setTimeout(() => {
      setIsGeneratingReport(false)
      setShowReportModal(false)
      showSimpleNotification('success', 'Rapport généré', `Le rapport ${selectedReportType} a été généré avec succès`)
      
      // Simuler le téléchargement
      const content = `Rapport ${selectedReportType} - ${new Date().toLocaleDateString('fr-FR')}\n\n`
      const blob = new Blob([content], { type: 'text/plain' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `rapport-${selectedReportType}-${new Date().toISOString().split('T')[0]}.txt`
  ...
  const handleExportData = (type: string, format: string) => {
    ...
    // Simuler l'export avec téléchargement réel
    setTimeout(() => {
      setIsExporting(false)
      
      // Créer un fichier de démonstration
      const content = `Données ${type} - ${new Date().toLocaleDateString('fr-FR')}\n\n`
      const blob = new Blob([content], { type: 'text/plain' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `export-${type}-${format}.${format === 'pdf' ? 'pdf' : 'txt'}`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-900">{formatNumber(salesData.reduce((sum, item) => sum + item.sales, 0))}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xl font-bold text-blue-900">{displayAmountWithPoints(salesData.reduce((sum, item) => sum + item.revenue, 0))}</p>
              </div>
        ...
        <Card className="hover:shadow-lg transition-all duration-300 border-l-4 border-l-[#10b981] bg-gradient-to-br from-green-50 to-green-100/50">
          <CardContent className="p-4">
            ...
            <Progress value={32} className="h-2 mb-2 bg-green-100" />
            <div className="flex items-center space-x-2">
              {getGrowthIcon(2.1)}
              ...
              {salesData.map((item, index) => (
                <div key={index} className="flex flex-col items-center space-y-2">
                  <div
                    ...
                    style={{ height: `${(item.sales / 2000) * 200}px` }}
                    onClick={() => handleViewProductDetails(index + 1)}
                  />
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button 
                variant="outline" 
                onClick={() => handleExportData('summary', 'pdf')}
              ...
              <Button 
                variant="outline" 
                onClick={() => handleExportData('detailed', 'excel')}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Client #{selectedCustomer}</h3>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{displayAmountWithPoints(45600)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Badge className="bg-[#10b981]/20 text-[#10b981] border-[#10b981]/30">Actif</Badge>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex space-x-2 mt-2">
                  <Button
                    ...
                    onClick={() => handleGenerateReportConfirm()}
