...
export default function ProductsPage() {
  ...
  const handleShare = (productId: number, platform: string) => {
  ...
  const handleBuyWithPoints = (productId: number) => {
    const product = products.find(p => p.id === productId)
    if (product) {
  ...
  const handleProductClick = (product: any) => {
    console.log('🚀 === DÉBUT handleProductClick ===')
    console.log('🔍 handleProductClick appelé avec:', product)
    ...
    console.log('📦 Produit complet créé:', fullProduct)
    console.log('🔧 Avant setSelectedProduct - selectedProduct:', selectedProduct)
    console.log('🔧 Avant setIsModalOpen - isModalOpen:', isModalOpen)
    
    console.log('🔧 Avant setSelectedProduct - selectedProduct actuel:', selectedProduct)
    console.log('🔧 Avant setIsModalOpen - isModalOpen actuel:', isModalOpen)
    
    console.log('🔧 Avant setSelectedProduct - selectedProduct actuel:', selectedProduct)
    console.log('🔧 Avant setIsModalOpen - isModalOpen actuel:', isModalOpen)
    
    // Utiliser des fonctions de mise à jour pour éviter les problèmes de closure
    setSelectedProduct((prev: any) => {
      console.log('🔧 setSelectedProduct appelé avec prev:', prev, 'et new:', fullProduct)
      return fullProduct
    })
    
    setIsModalOpen((prev: boolean) => {
      console.log('🔧 setIsModalOpen appelé avec prev:', prev, 'et new: true')
      return true
    })
    
    console.log('✅ Après setSelectedProduct et setIsModalOpen')
    console.log('🔧 selectedProduct sera:', fullProduct)
    console.log('🔧 isModalOpen sera: true')
    
    // Vérification immédiate des états
    setTimeout(() => {
      console.log('⏰ Vérification après 100ms:')
      console.log('🔧 selectedProduct actuel:', selectedProduct)
      console.log('🔧 isModalOpen actuel:', isModalOpen)
    }, 100)
  }

  const handleStartChat = (product: any) => {
    // Ouvrir le modal chat avec les informations du vendeur et du produit
    setChatSellerId(product.seller?.toLowerCase().replace(/\s+/g, '-') || 'vendeur-probooster')
    setChatSellerName(product.seller || 'Vendeur Probooster')
    setChatSellerAvatar('/placeholder-user.jpg') // Avatar par défaut
    setSelectedProduct(product) // Définir le produit sélectionné pour le chat
    setShowChatModal(true)
    console.log('💬 Chat démarré pour le produit:', product.name, 'avec le vendeur:', product.seller)
  }
