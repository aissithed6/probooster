...
/**
 * GET /api/super-admin/carriers — Liste des transporteurs (Super Admin).
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      const message = error.message ?? error.hint ?? error.details ?? 'Impossible de charger les transporteurs.'
      return NextResponse.json({ error: message }, { status: 500 })
    }

    const carriers = (data ?? []).map((row: any) => ({
      id: String(row?.id ?? ''),
