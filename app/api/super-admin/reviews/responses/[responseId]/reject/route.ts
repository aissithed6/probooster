...
/**
 * POST /api/super-admin/reviews/responses/:responseId/reject
 * Rejette une réponse vendeur (product_review_responses).
 */
export async function POST(request: NextRequest, context: { params: Promise<{ responseId: string }> }) {
  try {
    ...
    if (updateErr) {
      return NextResponse.json({ error: updateErr.message ?? 'Impossible de rejeter la réponse.' }, { status: 500 })
    }
    ...
    try {
      await supabase.from('product_review_moderation_events').insert({
        ...
        payload: { response_id: responseId, reason: reason || null }
      } as any)
    } catch {
      // ignore
    }
