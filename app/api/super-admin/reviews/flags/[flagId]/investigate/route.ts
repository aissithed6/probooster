...
/**
 * POST /api/super-admin/reviews/flags/:flagId/investigate
 * Marque un signalement (product_review_flags) comme "investigating".
 */
export async function POST(request: NextRequest, context: { params: Promise<{ flagId: string }> }) {
  try {
    ...
    const { data: updated, error: updateErr } = await supabase
      ...
      .update({ status: 'investigating', investigating_at: nowIso, updated_at: nowIso })
      .eq('id', flagId)
      .select('*')
      .maybeSingle()

    if (updateErr) {
      return NextResponse.json({ error: updateErr.message ?? 'Impossible de passer le signalement en enquête.' }, { status: 500 })
    }
