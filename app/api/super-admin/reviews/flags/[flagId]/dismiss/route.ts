...
/**
 * POST /api/super-admin/reviews/flags/:flagId/dismiss
 * Marque un signalement (product_review_flags) comme "dismissed" (non fondé).
 */
export async function POST(request: NextRequest, context: { params: Promise<{ flagId: string }> }) {
  try {
    ...
    if (updateErr) {
      return NextResponse.json({ error: updateErr.message ?? 'Impossible de classer le signalement.' }, { status: 500 })
    }
