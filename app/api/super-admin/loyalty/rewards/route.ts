...
/**
 * CRUD des récompenses de fidélité (super admin).
...
export async function GET(_req: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erreur lecture récompenses.'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reward?.name || !reward?.reward_type || !reward?.value_type) {
      return NextResponse.json({ error: 'Champs "name", "reward_type" et "value_type" obligatoires.' }, { status: 400 })
    }
    const supabase = getSupabaseAdmin()
    const { data, error } = await supabase
      .from('loyalty_rewards')
      .insert({ ...reward, created_by: userId, updated_by: userId })
      .select('*')
      .single()
    if (error) throw error
    return NextResponse.json({ data })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur création récompense.'
    const status = /accès|token/i.test(message) ? 401 : 500
    return NextResponse.json({ error: message }, { status })
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erreur mise à jour récompense.'
    const status = /accès|token/i.test(message) ? 401 : 500
    return NextResponse.json({ error: message }, { status })
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erreur statut récompense.'
    const status = /accès|token/i.test(message) ? 401 : 500
    return NextResponse.json({ error: message }, { status })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erreur suppression récompense.'
    const status = /accès|token/i.test(message) ? 401 : 500
    return NextResponse.json({ error: message }, { status })
  }
}
