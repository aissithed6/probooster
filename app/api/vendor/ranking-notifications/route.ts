...
/**
 * GET /api/vendor/ranking-notifications
 * POST /api/vendor/ranking-notifications
 * Persiste les notifications/alertes "classements" en DB.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('❌ GET /api/vendor/ranking-notifications failed:', error)
      const help = String(error?.message ?? '').includes('relation') ? buildCreateTableHelp('vendor_ranking_notifications') : null
      return NextResponse.json({ error: 'Erreur lors de la récupération des notifications.', help }, { status: 500 })
    }

    return NextResponse.json({ data: data ?? [] }, { status: 200 })
  } catch (error) {
    console.error('❌ GET /api/vendor/ranking-notifications unexpected error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('❌ POST /api/vendor/ranking-notifications failed:', error)
      const help = String(error?.message ?? '').includes('relation') ? buildCreateTableHelp('vendor_ranking_notifications') : null
      return NextResponse.json({ error: 'Erreur lors de la création de la notification.', help }, { status: 500 })
    }

    return NextResponse.json({ data }, { status: 201 })
  } catch (error) {
    console.error('❌ POST /api/vendor/ranking-notifications unexpected error:', error)
    const message = error instanceof Error ? error.message : 'Erreur inconnue.'
    const status = message.toLowerCase().includes('accès') ? 403 : message.toLowerCase().includes('token') ? 401 : 500
    return NextResponse.json({ error: message }, { status })
  }
