...
/**
 * POST: Annule une demande de retrait de points.
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!Number.isFinite(pointsAmount) || pointsAmount <= 0) {
    return NextResponse.json({ error: 'Montant de points invalide.' }, { status: 400 })
  }
  ...
  // Résout user_id si la ligne stocke un user_profiles.id
  ...
  // Recrédite le solde + trace adjustment + marque cancelled
  ...
  const { error: txErr } = await supabase
    ...
    .insert({
      ...
      description: `Annulation retrait #${id} (contre-écriture)`,
      reference_id: id,
      created_at: nowIso
    })
