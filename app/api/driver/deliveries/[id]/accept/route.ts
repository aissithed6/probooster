...
/**
 * S'assure que le driver est participant du chat livraison lié à order_id.
 */
...
/**
 * POST /api/driver/deliveries/:id/accept — Le livreur accepte la livraison.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (['delivered', 'cancelled', 'failed'].includes(currentStatus)) {
      return NextResponse.json({ error: 'Cette livraison ne peut pas être acceptée dans son état actuel.' }, { status: 409 })
    }
    ...
    await supabase.from('delivery_events').insert({
      ...
      data: { actor: 'driver', driver_id: driverId }
    })
    ...
    await supabase.from('delivery_chat_messages').insert({
      conversation_id: conversationId,
      sender_id: driverId,
      content: 'Le livreur a accepté la livraison.',
      message_type: 'system'
    })
