...
import { NotificationContainer, NotificationProvider, useNotifications } from "@/components/ui/modern-notification"
  ...
  return (
    <Suspense fallback={null}>
      <NotificationProvider>
        <HomePageClientInner initialProducts={initialProducts} />
      </NotificationProvider>
    </Suspense>
  )
}

function HomePageClientInner({ initialProducts }: { initialProducts: any[] }) {
  ...
  const [cartItems, setCartItems] = useState<
    Array<{ id: string; name: string; price: number; points: number; quantity?: number }>
  >([])
  const [wishlistItems, setWishlistItems] = useState<Array<{ id: string; name: string; price: number; points: number }>>([])

  const [dbProducts, setDbProducts] = useState<any[]>(Array.isArray(initialProducts) ? initialProducts : [])
  ...
  useEffect(() => {
    ...
    const loadDbProducts = async () => {
      ...
      try {
        ...
        const url = '/api/public/products/popular?limit=12'
        ...
        if (cancelled) return
        console.error('Home loadDbProducts failed to fetch:', {
          url: '/api/public/products/popular?limit=12',
          error
        })
        setDbProducts([])
      } finally {
  ...
  const handleCompare = (product: any) => {
    try {
      ...
      const canAddToCompare = (newProduct: any, existingList: any[]): { canAdd: boolean; reason?: string } => {
        ...
        const hasSimilarProduct = existingList.some((existingProduct) => areProductsSimilar(newProduct, existingProduct))

        if (!hasSimilarProduct) {
          return {
            canAdd: false,
            reason:
              "Ce produit n'est pas similaire aux produits déjà en comparaison. Vous ne pouvez comparer que des produits de même marque, catégorie, domaine d'intérêt ou gamme de prix."
          }
        }
      ...
      if (canAdd) {
        ...
        window.dispatchEvent(
          new CustomEvent('compareListUpdated', {
  ...
  return (
    <PublicRoute>
      ...
      >
        <div className="mx-auto" style={{ maxWidth: '1200px', padding: '0 16px' }}>
          <div className="pt-4">
            <EditableMessagesBanner location="homepage" />
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Rejoignez{' '}
                <strong style={{ color: '#ff6600' }} className="heartbeat-animation">
                  Probooster
                </strong>
                , la marketplace où chaque achat devient une aventure sociale. Système de points innovant, interactions enrichies et technologie de pointe au service de votre expérience.
              </p>
              <div
                style={{
                  display: 'flex',
        ...
        <section style={{
          ...
          <div style={{
            ...
            {isLoadingDbProducts ? (
              <div style={{ padding: '16px 0', color: '#535455', textAlign: 'center' }}>Chargement des produits populaires…</div>
            ) : dbProducts.length === 0 ? (
              <div style={{ padding: '16px 0', color: '#535455', textAlign: 'center' }}>Aucun produit populaire pour le moment.</div>
            ) : (
              <div style={{
                display: 'grid',
                    ...
                    product={
                      {
                        ...
                        seller: (product as any)?.sellerName ?? product.seller ?? 'Boutique',
                        vendorId: product.vendorId ?? undefined,
                        sharePoints: product.sharePoints ?? 50,
                        shares: (product as any)?.shareCount ?? product.shares ?? 0,
                        ...
                        sales: (product as any)?.totalSales ?? (product as any)?.sales ?? 0
                      } as any
                    }
      ...
      <section style={{
        ...
        {isModalOpen && selectedProduct && (
          <ProductModal product={selectedProduct} isOpen={isModalOpen} onClose={handleCloseProductModal} />
        )}
