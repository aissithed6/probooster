...
/**
 * Carte vendeur (UI existante) avec présence + résumé vendeur dynamiques.
 */
function FeaturedSellerCard({
  seller,
  index,
  onViewProducts,
  onOpenChat
  ...
  seller: any
  index: number
  onViewProducts: (s: any) => void
  onOpenChat: (s: any) => void
}) {
  const vendorId = typeof seller?.id === 'string' ? String(seller.id).trim() : ''
  const resolvedVendorId = UUID_REGEX.test(vendorId) ? vendorId : ''
      ...
      <CardHeader className="pb-6 relative z-10">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className={`text-xs font-semibold mt-1 ${isSellerOnline ? 'text-green-700' : 'text-gray-500'}`}>
      ...
      <CardContent className="space-y-6 relative z-10">
        ...
        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border border-yellow-200">
          <div className="flex items-center space-x-2">
            <div className="flex items-center">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`h-5 w-5 ${i < Math.floor(Number(ratingValue ?? 0)) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                />
              ))}
            </div>
          ...
          <div className="text-right">
            <div className="text-2xl font-bold text-[#ff6600]">{Number(ratingValue ?? 0).toFixed(1)}</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200 hover:shadow-md transition-all duration-300">
            <div className="text-2xl font-bold text-green-600 mb-1">{Number(seller.sales ?? 0).toLocaleString()}</div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {seller.specialties.map((specialty: string, idx: number) => (
              <Badge
                key={idx}
                variant="outline"
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-gray-200">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="text-sm font-semibold text-gray-700">{responseTimeLabel}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
        ...
        <div className="flex space-x-3 pt-2">
          ...
          <Button
            size="sm"
            onClick={() => onViewProducts(seller)}
            className="flex-1 bg-gradient-to-r from-[#ff6600] to-orange-500 hover:from-orange-600 hover:to-[#ff6600] text-white border-0 shadow-lg group-hover:scale-105 transition-all duration-300 font-medium"
          >
            Voir
...
export default function SellersPage() {
  ...
  const [apiVendors, setApiVendors] = useState<PublicVendor[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/public/vendors/list', { method: 'GET' })
        ...
        const parsed = raw
          .map((v: any) => ({
            ...
            avatar: String(v?.avatar ?? '').trim(),
            role: String(v?.role ?? '').trim()
          }))
          .filter((v: PublicVendor) => UUID_REGEX.test(v.id))

        if (!cancelled) setApiVendors(parsed)
      } catch {
        if (!cancelled) setApiVendors([])
      }
    })()
  ...
  const sellersForUi = useMemo(() => {
    if (!apiVendors.length) return featuredSellers

    return featuredSellers.map((s: any, idx: number) => {
      const v = apiVendors[idx]
      if (!v) return s
      ...
      return {
        ...
        id: v.id,
        name: v.name || s.name,
        avatar: v.avatar || s.avatar,
        initials: buildInitials(v.name || s.name)
      }
    })
  }, [apiVendors, featuredSellers])

  const filteredSellers = sellersForUi.filter((seller: any) => {
