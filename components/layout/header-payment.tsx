...
export default function HeaderPayment() {
  ...
  const activeMethods = useMemo(() => {
    const methods = publicSettings?.paymentMethods ?? []
    return methods.filter((m) => Boolean(m?.isActive) && Boolean(m?.id) && Boolean(m?.name))
  }, [publicSettings?.paymentMethods])

  const selectedActiveMethod = useMemo(() => {
    if (!activeMethods.length) return null
    return activeMethods.find((m) => m.id === paymentMethod) ?? null
  ...
  const renderMethodIcon = (type: string) => {
    switch (type) {
      case 'mobile_money':
        return <Wallet className="h-8 w-8 text-[#ff6600] mx-auto mb-2" />
      case 'bank_transfer':
        return <ArrowRight className="h-8 w-8 text-blue-600 mx-auto mb-2" />
      case 'bank_card':
        return <CreditCard className="h-8 w-8 text-[#ff6600] mx-auto mb-2" />
      case 'crypto':
        return <Zap className="h-8 w-8 text-purple-600 mx-auto mb-2" />
      case 'cash':
        return <Wallet className="h-8 w-8 text-green-600 mx-auto mb-2" />
      default:
        return <CreditCard className="h-8 w-8 text-[#ff6600] mx-auto mb-2" />
    }
  }
  ...
  useEffect(() => {
    ...
    const hasSelected = activeMethods.some((m) => m.id === paymentMethod)
      ...
      <Dialog open={showPaymentModal} onOpenChange={setShowPaymentModal}>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {activeMethods.length ? (
                activeMethods.map((method) => (
                  <Card
                    key={method.id}
                    className={`cursor-pointer border-2 ${paymentMethod === method.id ? 'border-[#ff6600]' : 'border-gray-200'}`}
                    onClick={() => handlePaymentMethodSelect(method.id)}
                  >
                    <CardContent className="p-4 text-center">
                      {method.logo ? (
                        <img src={method.logo} alt={`${method.name} Logo`} className="h-8 w-8 object-contain mx-auto mb-2" />
                      ) : (
                        renderMethodIcon(method.type)
                      )}
                      <h4 className="font-semibold">{method.name}</h4>
                      <p className="text-sm text-gray-600">{method.provider || 'Paiement'}</p>
                      <Badge className="mt-2 bg-green-100 text-green-800">Actif</Badge>
                    </CardContent>
                  </Card>
                ))
              ) : (
            ...
            {!activeMethods.length && paymentMethod === 'installment' && (
              <Card>
                <CardHeader>
            ...
            {!activeMethods.length && paymentMethod === 'deferred' && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
            ...
            <div className="flex justify-between pt-4">
              ...
              {(!activeMethods.length ? paymentMethod === 'card' : Boolean(selectedActiveMethod)) && (
                <Button onClick={handlePaymentConfirm} className="bg-[#ff6600] hover:bg-[#e55a00]">
                  <Lock className="h-4 w-4 mr-2" />
                  {activeMethods.length && selectedActiveMethod ? `Procéder au paiement - ${selectedActiveMethod.name}` : 'Procéder au paiement sécurisé'}
                </Button>
              )}
            </div>
          </div>
