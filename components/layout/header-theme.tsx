...
export default function HeaderTheme() {
  const { systemPrefs, setTheme } = useUserPreferences()
  ...
  useEffect(() => {
    setCurrentTheme(systemPrefs.theme)
  }, [systemPrefs.theme])
  ...
  const loadThemeSettings = () => {
    try {
      ...
      // Le thème est désormais centralisé via UserPreferencesProvider.
      // On ne ré-applique pas ici pour éviter les conflits.
    } catch (error) {
  ...
  const applyTheme = (theme: "light" | "dark" | "auto") => {
    // Le thème est centralisé via UserPreferencesProvider (Supabase + DOM).
    // On conserve l'event pour compatibilité avec d'autres composants.
    setTheme(theme)
