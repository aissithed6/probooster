...
export default function HeaderCompare() {
  ...
  const { purchaseValue, refresh: refreshClientPoints } = useClientPoints()

  /**
   * Convertit un montant FCFA vers des points selon la configuration Super Admin.
   * Règle: 1 point = purchaseValue FCFA => points = FCFA / purchaseValue.
   */
  const convertFCFAToPoints = (amountFcfa: number) => {
    const rate = Number(purchaseValue)
    const safeRate = Number.isFinite(rate) && rate > 0 ? rate : 1
    return Math.round((Number(amountFcfa) || 0) / safeRate)
  }
  ...
  useEffect(() => {
    ...
    // Retard = 0: on force un refresh des configs points au montage (cache déjà pris en compte).
    try {
  ...
  const removeFromCompare = (productId: number) => {
    try {
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('compareListUpdated', { detail: { compareList: updatedList, length: updatedList.length } }))
      }
      addNotification({ 
  type: 'success', 
  title: 'Succès', 
  ...
  const clearCompareList = () => {
    try {
      ...
      setCompareListLength(0)
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('compareListUpdated', { detail: { compareList: [], length: 0 } }))
      }
      addNotification({ 
  type: 'success', 
  title: 'Succès', 
    ...
    try {
      ...
      const pdfContent = {
        ...
        products: compareList.map((item, index) => ({
          ...
          name: item.name,
          price: item.price,
          points: convertFCFAToPoints(item.price),
  ...
  const addToCart = async (product: any) => {
    ...
    try {
      const before = CartService.getCart()
      CartService.addToCart(product)
      const after = CartService.getCart()
      ...
      if (Array.isArray(before) && Array.isArray(after) && after.length > before.length) {
        ...
        addNotification({ type: 'info', title: 'Information', message: 'Quantité mise à jour dans le panier' })
      }
                      ...
                      <td key={item.id} className="px-4 py-3 text-center">
                        <div className="space-y-3">
                          ...
                          <div className="bg-yellow-50 rounded-lg p-2 border border-yellow-200 hover:bg-yellow-100 transition-colors duration-300">
                            ...
                            <p className="text-lg font-bold text-yellow-800 hover:scale-110 transition-transform duration-300 inline-block">{convertFCFAToPoints(item.price)} pts</p>
                          </div>
