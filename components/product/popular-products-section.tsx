...
export default function PopularProductsSection({ onProductClick }: PopularProductsSectionProps) {
  ...
  const [shareCountsMap, setShareCountsMap] = useState<Record<string, { total: number }>>({})

  useEffect(() => {
    let mounted = true
    const pids = popularProducts.map(p => String(p.id).trim()).filter(Boolean)
    ...
    const loadAllStats = async () => {
      try {
        ...
        if (!mounted) return
        const newMap: Record<string, { total: number }> = {}
        pids.forEach((pid, idx) => {
          newMap[pid] = { total: results[idx].total }
        })
  ...
  const handleShare = (product: Product, platform: string) => {
    const pointsPrice = Math.max(0, Math.round(Number(product.price || 0) / purchaseValue))
    const shareText = `Découvrez ${product.name} sur Probooster ! Prix: ${product.price} F CFA, Points: ${pointsPrice}`
              ...
              >
                <AdvancedProductCard 
                  ...
                  product={{
                    ...
                    sharePoints: 0, 
                    shares: shareCountsMap[String(product.id)]?.total || 0
                  }}
                  onBuyWithPoints={() => handleBuyWithPoints(product)}
