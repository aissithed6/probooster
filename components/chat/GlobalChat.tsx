...
export const GlobalChat: React.FC = () => {
  ...
  useEffect(() => {
    // Scroll auto vers le bas à chaque nouveau message (suivi temps réel)
    try {
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden bg-white">
                  ...
                  <button
                    onClick={() => {
                      ...
                      toast({
                        ...
                        description: "Ce contact a été bloqué",
                        variant: "default",
                      })
                      setShowMoreMenu(false)
                    }}
      ...
      <div className="flex-1 overflow-hidden min-h-0">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'conversations' | 'produits')} className="h-full flex flex-col min-h-0 overflow-hidden">
          ...
          <TabsContent value="conversations" className="flex-1 min-h-0 h-full data-[state=active]:flex data-[state=active]:flex-col overflow-hidden">
            ...
            {/* Messages */}
            <div className="flex-1 min-h-0 h-full overflow-y-auto px-4">
                    ...
                    <div className={`flex-1 ${message.sender === 'user' ? 'text-right' : ''}`}>
                      ...
                      {message.type === 'document' && (
                        <div className={`p-3 rounded-lg shadow-sm ${
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className={`text-sm ${
                                ...
                                {Number.isFinite(Number(message.fileSize)) ? `${Math.round(Number(message.fileSize) / 1024)} KB` : ''}
                              </p>
                            </div>
                            {message.fileUrl && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className={`text-sm underline ${message.sender === 'user' ? 'text-white' : 'text-orange-600'}`}
                              >
                                Télécharger
                              </a>
                            )}
                          ...
                          {String((message as any)?.fileType ?? '').startsWith('audio/') && message.fileUrl && (
                            <div className="mt-3">
