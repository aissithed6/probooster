-- ============================================
-- SYSTÈME MARKETING ET PROMOTIONS - SUPABASE
-- ============================================
-- Date: 2025-10-07
-- Description: Système complet de boostage de produits et promotions
-- ============================================
...
-- Table des services de boostage créés par les admins
...
-- Index pour les services actifs
...
-- Table des campagnes de boostage achetées par les vendeurs
CREATE TABLE IF NOT EXISTS boosting_campaigns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  vendor_id UUID REFERENCES users(id) NOT NULL,
  product_id UUID, -- Optionnel, NULL pour boostage général du vendeur
  ...
  target_pages JSONB DEFAULT '[]', -- Pages où afficher le boost
...
CREATE TABLE IF NOT EXISTS boosting_performance (
  ...
  conversion_rate DECIMAL(5,2) DEFAULT 0, -- Taux de conversion (%)
  revenue DECIMAL(10,2) DEFAULT 0, -- Revenu généré
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Table des promotions créées par les admins
CREATE TABLE IF NOT EXISTS promotions (
  ...
  usage_limit INTEGER, -- Limite d'utilisation globale
  usage_limit_per_user INTEGER DEFAULT 1, -- Limite par utilisateur
  used_count INTEGER DEFAULT 0,
  target_audience JSONB DEFAULT '[]', -- ['all', 'new_customers', 'vip', etc.]
  applicable_products JSONB DEFAULT '[]', -- IDs des produits concernés
  applicable_categories JSONB DEFAULT '[]', -- Catégories concernées
  applicable_vendors JSONB DEFAULT '[]', -- Vendeurs concernés
  is_auto_apply BOOLEAN DEFAULT false, -- Application automatique
  created_by UUID REFERENCES users(id),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ============================================
-- 5. UTILISATION DES PROMOTIONS
-- ============================================

-- Table de suivi de l'utilisation des promotions
CREATE TABLE IF NOT EXISTS promotion_usage (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  promotion_id UUID REFERENCES promotions(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES users(id) NOT NULL,
  order_id UUID, -- Référence à la commande (à créer)
...
-- Index pour l'utilisation des promotions
...
-- ============================================
-- 6. CONFIGURATION DES PRIX DE BOOSTAGE
-- ============================================

-- Table de configuration des prix par page/pays
CREATE TABLE IF NOT EXISTS boosting_pricing (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  service_id UUID REFERENCES boosting_services(id) ON DELETE CASCADE NOT NULL,
  page_type VARCHAR(100), -- 'homepage', 'category', 'search', etc.
  country_code VARCHAR(10), -- Code pays pour WhatsApp
...
-- Politiques pour boosting_services
...
-- Politiques pour boosting_campaigns
...
-- Politiques pour boosting_performance
...
-- Politiques pour promotion_usage
...
-- Politiques pour boosting_pricing
...
CREATE OR REPLACE FUNCTION calculate_ctr(p_impressions INTEGER, p_clicks INTEGER)
RETURNS DECIMAL(5,2) AS $$
BEGIN
  IF p_impressions = 0 THEN
    RETURN 0;
  END IF;
  RETURN ROUND((p_clicks::DECIMAL / p_impressions::DECIMAL) * 100, 2);
END;
$$ LANGUAGE plpgsql;

-- Fonction pour calculer le taux de conversion
CREATE OR REPLACE FUNCTION calculate_conversion_rate(p_clicks INTEGER, p_conversions INTEGER)
RETURNS DECIMAL(5,2) AS $$
BEGIN
  IF p_clicks = 0 THEN
    RETURN 0;
  END IF;
  RETURN ROUND((p_conversions::DECIMAL / p_clicks::DECIMAL) * 100, 2);
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER update_boosting_services_updated_at BEFORE UPDATE ON boosting_services
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_boosting_campaigns_updated_at BEFORE UPDATE ON boosting_campaigns
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_promotions_updated_at BEFORE UPDATE ON promotions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Trigger pour incrémenter used_count des promotions
...
CREATE TRIGGER increment_promotion_usage_trigger AFTER INSERT ON promotion_usage
