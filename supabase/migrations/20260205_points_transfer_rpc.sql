-- Création / standardisation de la RPC de transfert de points
-- Objectif: rendre l'appel supabase.rpc('transfer_points_between_users', { p_sender_id, p_recipient_id, p_points }) fonctionnel et sécurisé.

alter table if exists public.loyalty_points
  add column if not exists is_frozen boolean not null default false;

alter table if exists public.loyalty_points
  add column if not exists frozen_at timestamptz;

alter table if exists public.loyalty_points
  add column if not exists frozen_by uuid;

alter table if exists public.loyalty_points
  add column if not exists freeze_reason text;

create or replace function public.transfer_points_between_users(
  p_sender_id uuid,
  p_recipient_id uuid,
  p_points integer
)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  v_sender_user_id uuid;
  v_recipient_user_id uuid;
  v_sender_balance integer;
  v_sender_frozen boolean;
  v_is_admin boolean;
  v_rate numeric;
begin
  if p_points is null or p_points <= 0 then
    raise exception 'Le montant du transfert doit être positif';
  end if;

  if p_sender_id is null or p_recipient_id is null then
    raise exception 'Identifiants invalides';
  end if;

  if p_sender_id = p_recipient_id then
    raise exception 'Impossible de se transférer des points à soi-même';
  end if;

  -- Résolution robuste: l'app peut passer un ID user (auth/users) ou un ID de profil.
  select coalesce((select up.user_id from public.user_profiles up where up.id = p_sender_id), p_sender_id)
    into v_sender_user_id;

  select coalesce((select up.user_id from public.user_profiles up where up.id = p_recipient_id), p_recipient_id)
    into v_recipient_user_id;

  if v_sender_user_id = v_recipient_user_id then
    raise exception 'Impossible de se transférer des points à soi-même';
  end if;

  if auth.uid() is null then
    raise exception 'Utilisateur non authentifié';
  end if;

  if not exists (select 1 from public.users u where u.id = v_sender_user_id) then
    raise exception 'Expéditeur introuvable';
  end if;

  if not exists (select 1 from public.users u where u.id = v_recipient_user_id) then
    raise exception 'Destinataire introuvable';
  end if;

  -- Autorisation: l'expéditeur ou un admin/super_admin peut déclencher le transfert.
  v_is_admin := false;
  if auth.uid() <> v_sender_user_id then
    if not exists (
      select 1
      from public.users u
      where u.id = auth.uid()
        and u.role in ('admin', 'super_admin')
    ) then
      raise exception 'Accès refusé';
    end if;
    v_is_admin := true;
  else
    if exists (
      select 1
      from public.users u
      where u.id = auth.uid()
        and u.role in ('admin', 'super_admin')
    ) then
      v_is_admin := true;
    end if;
  end if;

  -- Taux de conversion: utilisé pour maintenir fcfa_value synchronisé.
  v_rate := 1;
  if to_regclass('public.point_settings') is not null then
    execute 'select coalesce((select ps.conversion_rate from public.point_settings ps where ps.scope = ''global'' limit 1), 1)'
      into v_rate;
  end if;

  -- S'assure que les comptes de points existent.
  insert into public.loyalty_points (user_id)
  values (v_sender_user_id)
  on conflict (user_id) do nothing;

  insert into public.loyalty_points (user_id)
  values (v_recipient_user_id)
  on conflict (user_id) do nothing;

  -- Verrouillage + vérification du solde expéditeur.
  select lp.points_balance,
         coalesce(lp.is_frozen, false)
    into v_sender_balance,
         v_sender_frozen
  from public.loyalty_points lp
  where lp.user_id = v_sender_user_id
  for update;

  if v_sender_frozen and not v_is_admin then
    raise exception 'Compte gelé : transfert impossible';
  end if;

  if v_sender_balance is null or v_sender_balance < p_points then
    raise exception 'Solde insuffisant pour effectuer le transfert';
  end if;

  -- Débit expéditeur: le code applicatif complètera ensuite les frais (transfer_fee) si nécessaire.
  update public.loyalty_points
    set points_balance = points_balance - p_points,
        points_spent = coalesce(points_spent, 0) + p_points,
        fcfa_value = coalesce(fcfa_value, 0) - (p_points::numeric * v_rate)
  where user_id = v_sender_user_id;

  -- Crédit destinataire.
  update public.loyalty_points
    set points_balance = points_balance + p_points,
        points_earned = coalesce(points_earned, 0) + p_points,
        fcfa_value = coalesce(fcfa_value, 0) + (p_points::numeric * v_rate)
  where user_id = v_recipient_user_id;
end;
$$;

revoke all on function public.transfer_points_between_users(uuid, uuid, integer) from public;
grant execute on function public.transfer_points_between_users(uuid, uuid, integer) to authenticated;
grant execute on function public.transfer_points_between_users(uuid, uuid, integer) to service_role;

comment on function public.transfer_points_between_users(uuid, uuid, integer)
is 'Transfère des points entre deux utilisateurs. Accepte p_sender_id/p_recipient_id comme users.id (auth uid) ou user_profiles.id, et met à jour loyalty_points (débit/crédit).';

create or replace function public.admin_withdraw_points(
  p_target_id uuid,
  p_points integer,
  p_description text default null
)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  v_target_user_id uuid;
  v_balance integer;
  v_rate numeric;
begin
  if p_target_id is null then
    raise exception 'Identifiant utilisateur manquant';
  end if;

  if p_points is null or p_points <= 0 then
    raise exception 'Le nombre de points doit être positif';
  end if;

  if auth.uid() is null then
    raise exception 'Utilisateur non authentifié';
  end if;

  if not exists (
    select 1
    from public.users u
    where u.id = auth.uid()
      and u.role = 'super_admin'
  ) then
    raise exception 'Accès réservé au super administrateur';
  end if;

  select coalesce((select up.user_id from public.user_profiles up where up.id = p_target_id), p_target_id)
    into v_target_user_id;

  if not exists (select 1 from public.users u where u.id = v_target_user_id) then
    raise exception 'Utilisateur introuvable';
  end if;

  v_rate := 1;
  if to_regclass('public.point_settings') is not null then
    execute 'select coalesce((select ps.conversion_rate from public.point_settings ps where ps.scope = ''global'' limit 1), 1)'
      into v_rate;
  end if;

  insert into public.loyalty_points (user_id)
  values (v_target_user_id)
  on conflict (user_id) do nothing;

  select lp.points_balance
    into v_balance
  from public.loyalty_points lp
  where lp.user_id = v_target_user_id
  for update;

  if v_balance is null or v_balance < p_points then
    raise exception 'Solde insuffisant pour effectuer un retrait';
  end if;

  update public.loyalty_points
    set points_balance = points_balance - p_points,
        points_spent = coalesce(points_spent, 0) + p_points,
        fcfa_value = coalesce(fcfa_value, 0) - (p_points::numeric * v_rate)
  where user_id = v_target_user_id;

  insert into public.point_transactions (user_id, type, points, fcfa_value, description)
  values (
    v_target_user_id,
    'spend',
    p_points,
    (p_points::numeric * v_rate),
    coalesce(nullif(trim(p_description), ''), 'Retrait de points par le Super Admin')
  );
end;
$$;

revoke all on function public.admin_withdraw_points(uuid, integer, text) from public;
grant execute on function public.admin_withdraw_points(uuid, integer, text) to authenticated;

comment on function public.admin_withdraw_points(uuid, integer, text)
is 'Retire des points du compte loyalty_points d''un utilisateur (Super Admin). Accepte users.id ou user_profiles.id. Met à jour points_balance/points_spent/fcfa_value et ajoute une ligne point_transactions (type=spend).';

create or replace function public.set_points_freeze_status(
  p_target_id uuid,
  p_is_frozen boolean,
  p_reason text default null
)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  v_target_user_id uuid;
  v_now timestamptz;
begin
  if p_target_id is null then
    raise exception 'Identifiant utilisateur manquant';
  end if;

  if p_is_frozen is null then
    raise exception 'Statut gel manquant';
  end if;

  if auth.uid() is null then
    raise exception 'Utilisateur non authentifié';
  end if;

  if not exists (
    select 1
    from public.users u
    where u.id = auth.uid()
      and u.role = 'super_admin'
  ) then
    raise exception 'Accès réservé au super administrateur';
  end if;

  select coalesce((select up.user_id from public.user_profiles up where up.id = p_target_id), p_target_id)
    into v_target_user_id;

  if not exists (select 1 from public.users u where u.id = v_target_user_id) then
    raise exception 'Utilisateur introuvable';
  end if;

  v_now := now();

  insert into public.loyalty_points (user_id)
  values (v_target_user_id)
  on conflict (user_id) do nothing;

  perform 1
  from public.loyalty_points lp
  where lp.user_id = v_target_user_id
  for update;

  if p_is_frozen then
    update public.loyalty_points
      set is_frozen = true,
          frozen_at = v_now,
          frozen_by = auth.uid(),
          freeze_reason = nullif(trim(p_reason), '')
    where user_id = v_target_user_id;
  else
    update public.loyalty_points
      set is_frozen = false,
          frozen_at = null,
          frozen_by = null,
          freeze_reason = null
    where user_id = v_target_user_id;
  end if;
end;
$$;

revoke all on function public.set_points_freeze_status(uuid, boolean, text) from public;
grant execute on function public.set_points_freeze_status(uuid, boolean, text) to authenticated;

comment on function public.set_points_freeze_status(uuid, boolean, text)
is 'Gèle ou dégèle le compte de points d''un utilisateur (Super Admin). Accepte users.id ou user_profiles.id et met à jour is_frozen/frozen_at/frozen_by/freeze_reason dans loyalty_points.';
