-- Trace les opérations de gel / dégel dans point_transactions afin qu'elles apparaissent dans l'historique.
  ...
  if p_is_frozen then
    ...
    update public.loyalty_points
      set is_frozen = false,
          ...
          freeze_reason = null,
          points_balance = points_balance + v_freeze_amount,
          frozen_points = greatest(0, coalesce(frozen_points, 0) - v_freeze_amount)
    where user_id = v_target_user_id;
...
comment on function public.set_points_freeze_status(uuid, boolean, text, integer)
is 'Gèle ou dégèle le compte de points d''un utilisateur (Super Admin). Accepte users.id ou user_profiles.id et met à jour is_frozen/frozen_at/frozen_by/freeze_reason dans loyalty_points. Supporte gel partiel via p_points, met à jour points_balance/frozen_points et ajoute une trace dans point_transactions (type freeze/unfreeze).';
